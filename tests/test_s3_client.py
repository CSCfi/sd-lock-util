...
class TestS3Client(tests.mockups.SDLockUtilTestBase):
    ...
    async def test_s3_download_decrypted_object(self):

        mock_file = types.SimpleNamespace(write=unittest.mock.AsyncMock())
        mock_aiofiles = unittest.mock.Mock(return_value=self.mock_handler(mock_file))
        patch_aiofiles = unittest.mock.patch(
            "sd_lock_utility.s3_client.aiofiles.open", mock_aiofiles
        )
        ...
        async def fake_buffered_reader():
            yield b"123456789012foobarhelloworld"
            yield b"123456789012buzbozwellohorld"

        patch_streamingbody = unittest.mock.patch(
            "sd_lock_utility.s3_client.s3_buffered_reader",
            new=lambda *args, **kwargs: fake_buffered_reader(),
        )

        mock_decrypt = unittest.mock.Mock(return_value=b"a")
        patch_decrypt = unittest.mock.patch(
            "sd_lock_utility.s3_client.nacl.bindings.crypto_aead_chacha20poly1305_ietf_decrypt",
            mock_decrypt,
        )
        ...
        with patch_aiofiles, patch_streamingbody, patch_decrypt:
            await sd_lock_utility.s3_client.s3_download_decrypted_object(
                None,
                opts,
                session,
                {
                    "path": "test/file",
                    "localpath": "test/file",
                    "session_key": b"test-key",  # ensure bytes!
                },
                None,
            )

        mock_decrypt.assert_any_call(
            b"foobarhelloworld", None, b"123456789012", b"test-key"
        )
        mock_decrypt.assert_any_call(
            b"buzbozwellohorld", None, b"123456789012", b"test-key"
        )
